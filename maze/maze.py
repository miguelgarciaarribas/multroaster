# -*- coding: utf-8 -*-
# Based on https://github.com/boppreh.
# See LICENSE in this directory for copyright notice and license information
...
class Maze(object):
    ...
    def __repr__(self):
        return self._unicode_repr()

    def _serialized_repr(self):
        return self._get_maze(False)


    def _unicode_repr(self):
        matrix =  self._get_maze(True)
        for y, line in enumerate(matrix):
            for x, char in enumerate(line):
                if not g(x, y, matrix):
                    continue
                matrix[y][x] = Maze.UNICODE_BY_CONNECTIONS[matrix[y][x]]
        # Simple double join to transform list of lists into string.
        return '\n'.join(''.join(line) for line in matrix) + '\n'


    def _get_maze(self, formatToUnicode=True):
        """
        ...
        def _adapat_matrix(matrix):
            ...
            for y, line in enumerate(matrix):
                for x, char in enumerate(line):
                    if not g(x, y, matrix) and g(x - 1, y, matrix):
                        matrix[y][x - 1] = ' '
            return matrix
        matrix = skinny_matrix

        if formatToUnicode:
            matrix = _adapat_matrix(skinny_matrix)
        ...
        for y, line in enumerate(matrix):
            for x, char in enumerate(line):
                ...
                if not g(x, y + 1, matrix): connections.remove(S)
                if not g(x, y - 1, matrix): connections.remove(N)
                if not g(x + 1, y, matrix): connections.remove(E)
                if not g(x - 1, y, matrix): connections.remove(W)
